...
namespace Common.InputHooks
{
	...
	public abstract class GlobalHook
	{
		...
		public GlobalHook(ExitFunc exitFunc)
		{
			exitFunc.SetHook(this);
			exitFunc.Init();
		}
		...
		public void Start()
		{
			if (!this._isStarted && this._hookType != 0)
			{
				...
				this._hookCallback = this.HookCallbackProcedure;

				Assembly assembly = Assembly.GetExecutingAssembly();
				Module[] modules = assembly.GetModules();
				Module module = modules[0];
				IntPtr moduleHandle = Marshal.GetHINSTANCE(module);
				this._handleToHook = SetWindowsHookEx(this._hookType, this._hookCallback, moduleHandle, 0);
		...
		public void Stop()
		{
			if (this._isStarted)
			{
				UnhookWindowsHookEx(this._handleToHook);
		...
		public abstract class ExitFunc
		{
			private GlobalHook hook;

			internal void SetHook(GlobalHook globalHook)
			{
				Debug.Assert(this.hook == null);
